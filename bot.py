...
def get_main_keyboard():
    ...
    builder.row(InlineKeyboardButton(
        text="Запустить флуд",
        callback_data="start_flood_menu",
...
def get_accounts_keyboard():
    ...
    for phone in accounts:
        if accounts[phone]["client"]:
            status_emoji = "5891207662678317861"  # Человек с галочкой
        else:
            status_emoji = "5893192487324880883"  # Человек с крестиком
...
def get_active_accounts_keyboard(action):
    builder = InlineKeyboardBuilder()
    active_found = False
    for phone in accounts:
        if accounts[phone]["client"] and not accounts[phone]["flood_active"]:
            active_found = True
            builder.row(InlineKeyboardButton(
                text=phone,
                callback_data=f"{action}_{phone}",
                icon_custom_emoji_id="5870994129244131212"
            ))
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        '<tg-emoji emoji-id="6039422865189638057">📣</tg-emoji> Запускайте флуд\n'
        '<tg-emoji emoji-id="6037249452824072506">🔒</tg-emoji> Останавливайте флуд',
...
# Запуск флуда
@dp.callback_query(F.data == "start_flood_menu")
async def start_flood_menu(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        return
    
    keyboard = get_active_accounts_keyboard("floodacc")
    
    if not keyboard:
        await callback.message.edit_text(
            '<b><tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Нет подключенных аккаунтов для флуда</b>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await callback.answer()
    ...
    await callback.message.edit_text(
        '<b><tg-emoji emoji-id="6039422865189638057">📣</tg-emoji> Выберите аккаунт для флуда:</b>',
        parse_mode=ParseMode.HTML,
        reply_markup=keyboard
    )
    await state.set_state(FloodSetup.waiting_account)
    await callback.answer()

@dp.callback_query(FloodSetup.waiting_account, F.data.startswith("floodacc_"))
async def flood_account_selected(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        return
    
    phone = callback.data.replace("floodacc_", "")
    await state.update_data(phone=phone)
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="6039422865189638057">📣</tg-emoji> Запуск флуда для <code>{phone}</code></b>\n\n'
        'Отправьте команду в формате:\n'
        '<code>.флуд количество задержка сообщение</code>\n\n'
        'Пример: <code>.флуд 100 30 привет всем</code>',
        parse_mode=ParseMode.HTML,
        reply_markup=get_back_keyboard()
    )
    await state.set_state(FloodSetup.waiting_command)
    await callback.answer()

@dp.message(FloodSetup.waiting_command)
async def process_flood_command(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    ...
    if phone not in accounts or not accounts[phone]["client"]:
        await message.answer(
            '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Аккаунт не подключен',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await state.clear()
    ...
    if accounts[phone]["flood_active"]:
        await message.answer(
            '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Флуд уже запущен на этом аккаунте',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await state.clear()
    ...
    try:
        ...
        if len(parts) < 4 or parts[0] != ".флуд":
            await message.answer(
                '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Неверный формат. Используйте: .флуд количество задержка сообщение',
                parse_mode=ParseMode.HTML,
                reply_markup=get_back_keyboard()
            )
            return
        ...
        task = asyncio.create_task(flood_worker(accounts[phone]["client"], count, delay, text, phone))
        accounts[phone]["flood_task"] = task
        
        await state.clear()
        await message.answer(
            f'<b><tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Флуд запущен на {phone}!</b>\n\n'
