...
def em(key: str) -> str:
    """Создает премиум эмодзи тег для HTML"""
    return f'<tg-emoji emoji-id="{E[key]}">{get_emoji_char(key)}</tg-emoji>'

def get_emoji_char(key: str) -> str:
    """Возвращает текстовый символ эмодзи для отображения"""
    chars = {
        "settings": "⚙", "profile": "👤", "people": "👥", "file": "📁",
        "stats": "📊", "home": "🏘", "lock": "🔒", "unlock": "🔓",
        "megaphone": "📣", "check": "✅", "cross": "❌", "trash": "🗑",
        "back": "◁", "link": "🔗", "info": "ℹ", "bot": "🤖",
        "eye": "👁", "send": "⬆", "gift": "🎁", "clock": "⏰",
        "celebration": "🎉", "write": "✍", "apps": "📦", "code": "🔨",
        "loading": "🔄", "user_check": "👤", "wallet": "👛",
        "notification": "🔔", "smile": "🙂", "download": "⬇",
        "money": "🪙", "box": "📦", "pencil": "🖋",
    }
...
def get_main_keyboard():
    ...
    builder.button(text="Менеджер аккаунтов", icon_custom_emoji_id=E["settings"])
    builder.button(text="Функции", icon_custom_emoji_id=E["stats"])
    builder.button(text="Поддержка", icon_custom_emoji_id=E["megaphone"])
    builder.adjust(2, 1)
...
def get_accounts_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardBuilder().button(
            ...
            style="default",
            icon_custom_emoji_id=E["user_check"]
        ).as_markup().inline_keyboard[0]],
        [InlineKeyboardBuilder().button(
            text="Назад",
...
def get_functions_keyboard():
    ...
    builder.button(text="Проверка спам блока", callback_data="check_spam_block", style="danger", icon_custom_emoji_id=E["eye"])
...
def get_reactions_keyboard():
    ...
    for emoji_text, _ in BASE_REACTIONS:
        builder.button(text=emoji_text, callback_data="react_" + emoji_text, style="default")
    builder.button(text="Отмена", callback_data="cancel_reactions", style="danger", icon_custom_emoji_id=E["cross"])
...
async def accounts_manager(message: types.Message):
    ...
    text = (
        "<b>" + em('settings') + " Менеджер аккаунтов</b>\n"
...
async def add_account(callback: types.CallbackQuery, state: FSMContext):
    text = (
        "<b>" + em('apps') + " Добавление аккаунта</b>\n"
...
async def select_account_menu(callback: types.CallbackQuery):
    ...
    selected = user_selected_account.get(user_id, "Не выбран")
    text = (
        "<b>" + em('user_check') + " Выбор аккаунта</b>\n"
...
async def account_info(callback: types.CallbackQuery):
    ...
    text = (
        "<b>" + em('profile') + " Аккаунт:</b> <code>" + phone + "</code>\n"
...
async def check_spam_block(callback: types.CallbackQuery):
    ...
    try:
        # Ищем бота @spambot или создаем диалог
        ...
        for msg in messages:
            if msg.text and msg.from_id and hasattr(msg.from_id, 'user_id') and msg.from_id.user_id == spambot_entity.id:
                spam_result = msg.text
                break
        
        # Копируем текст и отправляем пользователю
        result_text = (
            "<b>" + em('eye') + " Результат проверки спам блока</b>\n\n"
            "<blockquote>" + spam_result[:1000] + "</blockquote>\n\n"
            + em('info') + " Аккаунт: <code>" + phone + "</code>"
        )
        
        # Очищаем историю с ботом
        try:
...
async def broadcast_menu(callback: types.CallbackQuery):
    ...
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=E["back"])
    builder.adjust(1)
    await safe_edit(callback.message, "<b>" + em('megaphone') + " Рассылка</b>", reply_markup=builder.as_markup())
    await callback.answer()
...
async def new_broadcast(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        "<b>" + em('write') + " Отправьте 1 сообщение для рассылки:</b>\n\n"
...
async def add_more_messages(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        "<b>" + em('write') + " Отправьте сообщение " + str(count + 1) + "/5:</b>\n\n"
        + em('info') + " Поддерживается HTML и премиум эмодзи"
    )
    await safe_edit(callback.message, text, reply_markup=get_back_keyboard("broadcast"))
    await callback.answer()
...
async def create_chat_selection_message(chat_id: int, state: FSMContext, page: int):
    ...
    if not chats:
        await safe_send(chat_id, em('cross') + " <b>Нет доступных чатов</b>", reply_markup=get_functions_keyboard())
        await state.clear()
        return
    ...
    text = (
        "<b>" + em('people') + " Выберите чаты (" + limit_text + "):</b>\n"
...
async def choose_broadcast_mode(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not selected_chats:
        ...
        return
    text = (
        "<b>" + em('send') + " Выберите режим рассылки:</b>\n\n"
...
async def start_broadcast_execution(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Всегда отправляем новое сообщение о запуске
    try:
        await callback.message.delete()
    except:
        pass
...
async def active_broadcasts_list(callback: types.CallbackQuery):
    ...
    if not broadcasts:
        ...
        text = "<b>" + em('unlock') + " Нет активных рассылок</b>"
        await safe_edit(callback.message, text, reply_markup=builder.as_markup())
        await callback.answer()
        return
    ...
    text = "<b>" + em('clock') + " Активные рассылки:</b> " + str(len(broadcasts))
    await safe_edit(callback.message, text, reply_markup=builder.as_markup())
    await callback.answer()
...
async def cancel_broadcast(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await safe_send(callback.message.chat.id, "<b>" + em('cross') + " Рассылка отменена</b>", reply_markup=get_functions_keyboard())
    await callback.answer()
...
async def join_chats_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        "<b>" + em('people') + " Вступление в чаты</b>\n\n"
...
async def process_join_usernames(message: types.Message, state: FSMContext):
    ...
    for i, username in enumerate(lines, 1):
        ...
        if i % 3 == 0 or i == len(lines):
            progress_text = (
                "<b>" + em('loading') + " Вступление в чаты...</b>\n"
                "Прогресс: " + str(i) + "/" + str(len(lines)) + "\n"
    ...
    result_text = (
        "<b>" + em('celebration') + " Вступление завершено!</b>\n\n"
...
async def mass_reactions_menu(callback: types.CallbackQuery):
    ...
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=E["back"])
    builder.adjust(1)
    await safe_edit(callback.message, "<b>" + em('smile') + " Масс реакции</b>", reply_markup=builder.as_markup())
    await callback.answer()
...
async def start_reactions(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not account_result:
        await safe_send(callback.message.chat.id, em('cross') + " Нет активного аккаунта", reply_markup=get_functions_keyboard())
        await state.clear()
        return
...
async def create_reaction_chat_selection(chat_id: int, state: FSMContext, page: int):
    ...
    text = (
        "<b>" + em('people') + " Выберите чаты для реакций (" + limit_text + "):</b>\n"
...
async def reaction_get_delay(message: types.Message, state: FSMContext):
    ...
    text = (
        "<b>" + em('send') + " Выберите тип работы:</b>\n\n"
...
async def reaction_set_type(callback: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit(callback.message, "<b>" + em('smile') + " Выберите реакцию:</b>", reply_markup=get_reactions_keyboard())
    await callback.answer()
...
async def reaction_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        "<b>" + em('smile') + " Масс реакции запущены!</b>\n"
        + em('people') + " Чатов: " + str(len(selected_chats)) + "\n"
...
async def active_reactions_list(callback: types.CallbackQuery):
    ...
    if not reactions:
        ...
        text = "<b>" + em('unlock') + " Нет активных реакций</b>"
        await safe_edit(callback.message, text, reply_markup=builder.as_markup())
        await callback.answer()
        return
    ...
    text = "<b>" + em('smile') + " Активные реакции:</b> " + str(len(reactions))
    await safe_edit(callback.message, text, reply_markup=builder.as_markup())
    await callback.answer()
...
async def cancel_reactions(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await safe_send(callback.message.chat.id, "<b>" + em('cross') + " Реакции отменены</b>", reply_markup=get_functions_keyboard())
    await callback.answer()
...
async def admin_broadcast_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        "<b>" + em('megaphone') + " Рассылка всем пользователям</b>\n\n"
...
async def admin_broadcast_get_message(message: types.Message, state: FSMContext):
    ...
    text = (
        "<b>" + em('send') + " Подтвердите рассылку</b>\n\n"
        + em('people') + " Пользователей: " + str(total_users) + "\n"
        + em('file') + " Сообщение:\n"
...
async def admin_broadcast_confirm(callback: types.CallbackQuery, state: FSMContext):
    ...
    result_text = (
        "<b>" + em('check') + " Рассылка завершена!</b>\n\n"
...
async def admin_broadcast_cancel(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await safe_edit(callback.message, "<b>" + em('cross') + " Рассылка отменена</b>", reply_markup=get_admin_keyboard())
    await callback.answer()
