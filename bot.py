...
async def handle_flood_command(client, message, phone):
    try:
        if accounts[phone]["flood_active"]:
            await message.reply(
                '❌ Флуд уже запущен! Остановите командой .стопфлуд'
            )
            return
        ...
        if len(parts) < 4:
            await message.reply(
                '❌ Формат: .флуд количество задержка сообщение\nПример: .флуд 100 30 привет всем'
            )
            return
        ...
        await message.reply(
            ...
            f'🔒 Остановить: .стопфлуд'
        )
        
    except ValueError:
        await message.reply('❌ Количество и задержка должны быть числами')

async def handle_stop_command(client, message, phone):
    if not accounts[phone]["flood_active"]:
        await message.reply('ℹ️ Флуд не запущен')
        return
    ...
    await message.reply('🔒 Флуд остановлен!')
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        '<b>🤖 Бот для управления флуд-аккаунтами</b>\n\n'
        '👤 Добавляйте Telegram аккаунты\n'
        '✍ Запускайте флуд на аккаунте командой <code>.флуд 100 30 текст</code>\n'
        '🔒 Останавливайте флуд здесь или командой <code>.стопфлуд</code>',
...
async def back_to_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        '<b>🤖 Бот для управления флуд-аккаунтами</b>\n\n'
        '👤 Добавляйте Telegram аккаунты\n'
        '✍ Запускайте флуд командой .флуд\n'
        '🔒 Останавливайте здесь или командой .стопфлуд',
        parse_mode=ParseMode.HTML,
        reply_markup=get_main_keyboard()
    )
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    if not re.match(r'^\+[0-9]{10,15}$', phone):
        await message.answer(
            '❌ Неверный формат номера. Используйте: +79123456789',
            reply_markup=get_back_keyboard()
        )
        return
    ...
    try:
        ...
        await message.answer(
            f'<b>👁 Код подтверждения отправлен на {phone}</b>\n\n'
...
async def process_code(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            f'ℹ️ Теперь можете запускать флуд на этом аккаунте командой:\n'
...
async def process_password(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f'<b>✅ Аккаунт {phone} успешно добавлен!</b>\n\n'
            f'ℹ️ Теперь можете запускать флуд командой:\n'
            f'<code>.флуд 100 30 ваш текст</code>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
...
async def my_accounts(callback: types.CallbackQuery):
    ...
    if not accounts:
        ...
        for phone, data in accounts.items():
            ...
            text += f'\n👤 <code>{phone}</code> - {status}{flood_text}'
...
async def connect_account(callback: types.CallbackQuery):
    ...
    if success:
        await callback.message.edit_text(
            f'<b>✅ Аккаунт {phone} подключен!</b>\n\n'
            f'ℹ️ Запускайте флуд командой <code>.флуд 100 30 текст</code> на этом аккаунте',
            parse_mode=ParseMode.HTML,
            reply_markup=get_account_actions_keyboard(phone)
        )
    else:
        await callback.message.edit_text(
            f'❌ Не удалось подключить {phone}',
            parse_mode=ParseMode.HTML,
            reply_markup=get_account_actions_keyboard(phone)
        )
...
async def stop_flood_menu(callback: types.CallbackQuery):
    ...
    if not keyboard:
        await callback.message.edit_text(
            '<b>ℹ️ Нет активных флудов</b>\n\n'
            '✍ Запустите флуд на аккаунте командой <code>.флуд 100 30 текст</code>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await callback.answer()
    ...
    await callback.message.edit_text(
        '<b>🔒 Выберите аккаунт для остановки флуда:</b>',
        parse_mode=ParseMode.HTML,
...
async def stop_flood_account(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f'<b>🔒 Флуд на {phone} остановлен!</b>',
        parse_mode=ParseMode.HTML,
        reply_markup=get_main_keyboard()
    )
    await callback.answer()
