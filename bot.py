...
async def reaction_next_step(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Далее - переход к вводу задержки"""
    data = await state.get_data()
    selected_chats = data.get("selected_chats", [])
    ...
    # Удаляем сообщение с кнопкой "Далее"
    try:
        await callback.message.delete()
    except:
        pass
    
    # Отправляем новое сообщение с запросом задержки
    text = em('write') + " <b>Введите задержку между реакциями (секунд):</b>"
    msg = await safe_send(user_id, text, reply_markup=get_back_keyboard("cancel_reactions"))
