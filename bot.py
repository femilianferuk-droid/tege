...
def get_functions_keyboard():
    ...
    builder.button(text="Команды", callback_data="commands_menu", style="success", icon_custom_emoji_id=E["code"])
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=E["back"])
    builder.adjust(1)
    return builder.as_markup()
...
def get_commands_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="Добавить команду", callback_data="add_command", style="primary", icon_custom_emoji_id=E["code"])
    builder.button(text="Мои команды", callback_data="my_commands", style="success", icon_custom_emoji_id=E["file"])
    builder.button(text="Приветствие новым", callback_data="welcome_setup", style="primary", icon_custom_emoji_id=E["gift"])
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=E["back"])
    builder.adjust(1)
    return builder.as_markup()
...
async def commands_menu(callback: types.CallbackQuery):
    ...
    # Убираем служебные команды для подсчета
...
async def welcome_setup(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        + em('info') + " Поддерживается HTML и премиум эмодзи\n"
        + em('info') + " Бот будет отвечать на ПЕРВОЕ сообщение от нового пользователя"
    )
    await safe_edit(callback.message, text, reply_markup=get_back_keyboard("commands_menu"))
    ...
    async def welcome_handler(event):
        ...
        try:
            ...
            if len(messages) <= 1:
                ...
                if welcome_msg:
                    try:
                        await client.send_message(sender_id, welcome_msg, parse_mode='html')
                    except Exception as ex:
...
async def my_commands_list(callback: types.CallbackQuery):
    ...
    if not display_cmds:
        ...
        builder = InlineKeyboardBuilder()
        builder.button(text="Назад", callback_data="commands_menu", style="default", icon_custom_emoji_id=E["back"])
        await safe_edit(callback.message, text, reply_markup=builder.as_markup())
        await callback.answer()
        return
...
async def edit_command(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="Удалить", callback_data="del_cmd_" + cmd_name, style="danger", icon_custom_emoji_id=E["trash"])
    builder.button(text="Назад", callback_data="my_commands", style="default", icon_custom_emoji_id=E["back"])
    builder.adjust(1)
