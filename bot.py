...
async def create_reaction_chat_selection(chat_id: int, state: FSMContext, page: int):
    ...
    if len(selected) >= 1:
        action_row.append(types.InlineKeyboardButton(
            ...
            callback_data="reaction_next_step"
        ))
    if len(selected) > 0:
        action_row.append(types.InlineKeyboardButton(
            text="🗑 Сбросить",
...
@dp.callback_query(F.data.startswith("rsc_"), ReactionStates.selecting_chats)
...
@dp.callback_query(F.data.startswith("rpg_"), ReactionStates.selecting_chats)
...
@dp.callback_query(F.data == "rclear_chats", ReactionStates.selecting_chats)
...
@dp.callback_query(F.data == "reaction_next_step", ReactionStates.selecting_chats)
...
# ============ BROADCAST (кратко, без изменений) ============
...
async def execute_broadcast(user_id, broadcast_id, chats, messages_list, count, delay, client, mode):
    ...
    logging.info("Broadcast done. Mode: " + mode + ", OK: " + str(completed) + ", ERR: " + str(errors))
...
async def join_chats_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        + em('write') + " Отправьте список юзернеймов:\n"
        "<code>@chat1\n@chat2\n@chat3</code>\n\n"
        + em('info') + " Задержка: 15 секунд"
    )
...
async def process_join_usernames(message: types.Message, state: FSMContext):
    ...
    for line in text.split('\n'):
        line = line.strip().replace('https://t.me/', '').replace('@', '').strip()
        if line:
            lines.append(line)

    if not lines:
        await safe_send(message.chat.id, em('cross') + " Не найдено юзернеймов", reply_markup=get_back_keyboard("functions"))
        return

    if len(lines) > 50:
        await safe_send(message.chat.id, em('cross') + " Максимум 50 чатов", reply_markup=get_back_keyboard("functions"))
        return
...
async def admin_broadcast_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = em('megaphone') + " <b>Рассылка всем</b>\n\n" + em('write') + " Отправьте сообщение с HTML"
...
async def admin_broadcast_get_message(message: types.Message, state: FSMContext):
    ...
    builder.adjust(2)
    await safe_send(message.chat.id, em('send') + " <b>Подтвердите рассылку</b>", reply_markup=builder.as_markup())
...
async def admin_broadcast_confirm(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        ...
        return
    data = await state.get_data()
    msg = data.get("admin_broadcast_message", "")
    sent = 0
...
async def admin_broadcast_cancel(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await safe_edit(callback.message, em('cross') + " Отменено", reply_markup=get_admin_keyboard())
    await callback.answer()
