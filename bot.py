...
async def show_chat_selection(message: types.Message, state: FSMContext, page: int):
    """Показывает страницу чатов с возможностью выбора до 10"""
    user_id = message.from_user.id
    ...
    selected: List[str] = data.get("selected_chats", [])
...
{em('info')} Нажимайте на чаты чтобы выбрать/снять
...
async def clear_chats_selection(callback: types.CallbackQuery, state: FSMContext):
    """Сбрасывает выбранные чаты и показывает тот же список"""
...
{em('clock')} Задержка: {data.get('delay', 1)}с"""
    await safe_edit(callback.message, text, reply_markup=builder.as_markup())
    await state.clear()
    await callback.answer(f"Рассылка запущена!", show_alert=True)
...
async def process_join_usernames(message: types.Message, state: FSMContext):
    ...
    # Убираем @ и https://t.me/ если есть
    lines = []
    ...
    for i, username in enumerate(lines, 1):
        try:
            # Пробуем через JoinChannelRequest
            try:
        ...
        # Обновляем прогресс каждые 3 чата или в конце
...
{em('cross')} Ошибок: {failed}"""
            try:
                ...
                logging.error(f"Error updating status: {ex}")

        # Задержка 15 секунд (кроме последнего)
        if i < len(lines):
