...
async def show_chat_selection(message: types.Message, state: FSMContext, page: int):
    """Показывает страницу чатов. Можно запустить рассылку даже с 1 чатом."""
    user_id = message.from_user.id
    ...
    # Кнопка запуска — доступна всегда, если выбран хотя бы 1 чат
    if len(selected) >= 1:
        builder.button(
    ...
    limit_text = f"до 10" if len(selected) < 10 else "ЛИМИТ"
...
{em('send')} Можно запустить с 1 чатом"""

    # Удаляем предыдущее сообщение со списком чатов если есть
