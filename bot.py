...
# ПРЕМИУМ ЭМОДЗИ - ПОЛНЫЕ ТЕГИ ДЛЯ СООБЩЕНИЙ
E = {
...
def process_placeholders(text: str, user_info: dict) -> str:
    """Заменяет плейсхолдеры {NICK}, {FIRSTNAME} и т.д. на реальные данные"""
    result = text
...
def get_functions_keyboard():
    ...
    builder.button(text="Вступление в чаты", callback_data="join_chats", style="success", icon_custom_emoji_id=eid("people"))
    builder.button(text="Масс реакции", callback_data="mass_reactions", style="primary", icon_custom_emoji_id=eid("smile"))
    builder.button(text="Проверка спам блока", callback_data="check_spam_block", style="danger", icon_custom_emoji_id=eid("eye"))
...
def get_reaction_type_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="Всё подряд", callback_data="rtype_all", style="primary", icon_custom_emoji_id=eid("send"))
    builder.button(text="Через один", callback_data="rtype_skip", style="success", icon_custom_emoji_id=eid("loading"))
    builder.button(text="Отмена", callback_data="cancel_reactions", style="danger", icon_custom_emoji_id=eid("cross"))
    builder.adjust(2, 1)
    return builder.as_markup()
...
async def cmd_start(message: types.Message):
    text = (
        ...
        + em('stats') + " <b>Функции</b> — рассылка, команды, реакции\n"
...
async def account_info(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    if not is_selected:
        builder.button(text="Выбрать", callback_data="sel_" + phone, style="success", icon_custom_emoji_id=eid("user_check"))
    builder.button(text="Удалить", callback_data="del_" + phone, style="danger", icon_custom_emoji_id=eid("trash"))
...
async def setup_command_handler(user_id: int):
    """Настраивает обработчик входящих сообщений для команд (отвечает ВСЕГДА)"""
    account = get_active_account(user_id)
    if not account:
        return
    ...
    async def command_handler(event):
        ...
        if msg_text in cmds:
            try:
                ...
                logging.error(f"Command reply error: {ex}")
...
async def setup_welcome_handler(user_id: int):
    ...
    async def welcome_handler(event):
        ...
        try:
            ...
            # Если есть только одно сообщение (текущее) - это новый пользователь
            if len(messages) <= 1:
                ...
                if welcome_msg:
                    ...
                    try:
                        ...
                        logging.error(f"Welcome message error: {ex}")
        except:
            pass
...
async def my_commands_list(callback: types.CallbackQuery):
    ...
    # Убираем служебную команду приветствия
    ...
    for cmd_name in display_cmds:
        builder.button(
            text=cmd_name,
            callback_data="edit_cmd_" + cmd_name,
            style="default"
        )
    builder.button(text="Назад", callback_data="commands_menu", style="default", icon_custom_emoji_id=eid("back"))
    builder.adjust(2)
...
async def mass_reactions_menu(callback: types.CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.button(text="Запустить реакции", callback_data="start_reactions", style="primary", icon_custom_emoji_id=eid("smile"))
    builder.button(text="Активные реакции", callback_data="active_reactions_list", style="default", icon_custom_emoji_id=eid("clock"))
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=eid("back"))
    builder.adjust(1)
...
async def create_reaction_chat_selection(chat_id: int, state: FSMContext, page: int):
    ...
    builder.button(text="Отмена", callback_data="cancel_reactions", style="default", icon_custom_emoji_id=eid("cross"))
    builder.adjust(1)
...
async def reaction_set_type(callback: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit(callback.message, em('smile') + " <b>Выберите реакцию:</b>", reply_markup=get_reactions_keyboard())
    await callback.answer()
...
async def reaction_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="Остановить", callback_data="rstop_" + reaction_id, style="danger", icon_custom_emoji_id=eid("cross"))
    builder.button(text="В меню", callback_data="main_menu", style="default", icon_custom_emoji_id=eid("home"))
    builder.adjust(1)
...
async def active_reactions_list(callback: types.CallbackQuery):
    ...
    if not reactions:
        builder = InlineKeyboardBuilder()
        builder.button(text="Назад", callback_data="mass_reactions", style="default", icon_custom_emoji_id=eid("back"))
