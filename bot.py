...
# Храним ID сообщений для удаления при пересоздании
...
async def delete_user_chat_messages(user_id: int):
    """Удаляет все предыдущие сообщения со списком чатов"""
...
async def broadcast_get_delay(message: types.Message, state: FSMContext):
    ...
    # Создаем первое сообщение со списком чатов (страница 0)
...
async def create_chat_selection_message(chat_id: int, state: FSMContext, page: int):
    """Создает НОВОЕ сообщение со списком чатов"""
    ...
    # Кнопки действий ВСЕГДА в одном ряду
    action_row = []
    if len(selected) >= 1:
...
async def select_chat(callback: types.CallbackQuery, state: FSMContext):
    """При выборе чата: удаляет старое сообщение и создает новое с обновленным списком"""
    parts = callback.data.replace("sc_", "").split("_")
    chat_id = parts[0] + "_" + parts[1] if len(parts) > 2 else parts[0]
    page = int(parts[-1]) if len(parts) > 2 else int(parts[1]) if len(parts) > 1 else 0

    # Определяем правильный chat_id_str
    if len(parts) > 2:
    ...
    # Удаляем ВСЕ старые сообщения со списком чатов
    await delete_user_chat_messages(user_id)

    # Удаляем текущее сообщение (то на которое нажали)
    try:
        await callback.message.delete()
    except:
        pass

    # Создаем НОВОЕ сообщение с обновленным списком
...
async def chats_page(callback: types.CallbackQuery, state: FSMContext):
    """Переключение страницы - удаляет старое и создает новое сообщение"""
...
async def clear_chats_selection(callback: types.CallbackQuery, state: FSMContext):
    """Сбрасывает выбранные чаты и пересоздает сообщение"""
    ...
    for i, username in enumerate(lines, 1):
        try:
            try:
                ...
                wait_time = e.seconds
                logging.warning(f"FloodWait {wait_time}s for {username}")
                ...
                try:
                    ...
                    logging.error(f"Failed after FloodWait: {username} - {e2}")
                ...
                try:
                    ...
                    failed += 1
                    failed_list.append(username)
                    logging.error(f"Failed to join {username}: {e2}")
        except Exception as e:
            failed += 1
            failed_list.append(username)
            logging.error(f"Outer error for {username}: {e}")
