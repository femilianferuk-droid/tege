...
from telethon.tl.types import (
    ...
    PeerUser, PeerChat, PeerChannel, InputPeerUser
)
...
AVAILABLE_COUNTRIES = ["UK", "US", "DE", "FR", "NL", "CA", "AU", "IT", "ES", "PL"]
...
# Хранилища для продажи аккаунтов
sell_accounts: Dict[int, Dict[str, List[Dict]]] = {}  # user_id -> {country: [{phone, client, 2fa_password}, ...]}
purchased_accounts: Dict[int, Dict[int, dict]] = {}  # user_id -> {purchase_number: {country, phone, client, 2fa_password}}
purchase_counter: Dict[int, int] = {}  # user_id -> counter
country_stock: Dict[int, Dict[str, int]] = {}  # user_id -> {country: count}
...
class SellStates(StatesGroup):
    waiting_for_sell_phone = State()
    waiting_for_sell_code = State()
    waiting_for_sell_2fa = State()
    waiting_for_sell_country = State()

class CheckAccountStates(StatesGroup):
    waiting_for_purchase_number = State()

# Премиум эмодзи для бота
E = {
    ...
    "shop": '<tg-emoji emoji-id="5904462880941545555">🪙</tg-emoji>',
    "key": '<tg-emoji emoji-id="6037249452824072506">🔒</tg-emoji>',
}
...
def process_placeholders(text: str, user_info: dict, command_data: dict = None) -> str:
    ...
    # Обработка плейсхолдеров стран
    if command_data:
        country_stock_data = command_data.get("country_stock", {})
        for country in AVAILABLE_COUNTRIES:
            placeholder = "{" + country + "}"
            if placeholder in result:
                count = country_stock_data.get(country, 0)
                result = result.replace(placeholder, str(count))
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        # Таблица для аккаунтов на продажу
        await conn.execute("""
        ...
        # Таблица для проданных аккаунтов
        await conn.execute("""
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_sell_accounts_user ON sell_accounts(user_id)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_sold_accounts_user ON sold_accounts(user_id)")
    
    logger.info("База данных инициализирована")

# Функции БД для продажи аккаунтов
async def save_sell_account_db(user_id: int, country: str, phone: str, session_file: str, has_2fa: bool = False, twofa_password: str = None):
    async with db_pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO sell_accounts (user_id, country, phone, session_file, has_2fa, twofa_password)
            VALUES ($1, $2, $3, $4, $5, $6)
        """, user_id, country, phone, session_file, has_2fa, twofa_password)

async def get_sell_accounts_db(user_id: int, country: str) -> List[dict]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT * FROM sell_accounts WHERE user_id=$1 AND country=$2 AND is_sold=FALSE
        """, user_id, country)
        return [dict(r) for r in rows]

async def get_country_stock_db(user_id: int) -> Dict[str, int]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT country, COUNT(*) as count FROM sell_accounts 
            WHERE user_id=$1 AND is_sold=FALSE GROUP BY country
        """, user_id)
        return {r['country']: r['count'] for r in rows}

async def mark_account_sold_db(user_id: int, account_id: int, purchase_number: int, buyer_id: int):
    async with db_pool.acquire() as conn:
        account = await conn.fetchrow("SELECT * FROM sell_accounts WHERE id=$1 AND user_id=$2", account_id, user_id)
        if account:
            await conn.execute("""
                INSERT INTO sold_accounts (user_id, purchase_number, country, phone, session_file, has_2fa, twofa_password, sold_to)
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
            """, user_id, purchase_number, account['country'], account['phone'],
                account['session_file'], account['has_2fa'], account['twofa_password'], buyer_id)
            await conn.execute("UPDATE sell_accounts SET is_sold=TRUE, purchase_number=$3 WHERE id=$1", account_id, purchase_number)

async def get_sold_account_db(user_id: int, purchase_number: int) -> Optional[dict]:
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT * FROM sold_accounts WHERE user_id=$1 AND purchase_number=$2
        """, user_id, purchase_number)
        return dict(row) if row else None

async def get_purchase_counter_db(user_id: int) -> int:
    async with db_pool.acquire() as conn:
        counter = await conn.fetchval("SELECT purchase_counter FROM users WHERE user_id=$1", user_id)
        return counter or 1

async def increment_purchase_counter_db(user_id: int) -> int:
    async with db_pool.acquire() as conn:
        await conn.execute("UPDATE users SET purchase_counter = purchase_counter + 1 WHERE user_id=$1", user_id)
        return await conn.fetchval("SELECT purchase_counter FROM users WHERE user_id=$1", user_id)
...
async def save_command_db(user_id: int, cmd: str, response: str, product_type: str = 'text', 
                          has_crypto: bool = False, amount: float = None, product: str = None,
                          success_message: str = None):
    async with db_pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO commands (user_id, command, response, product_type, success_message, has_crypto, crypto_amount, product_text) 
            VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
            ON CONFLICT (user_id, command) DO UPDATE SET 
                response=$3, product_type=$4, success_message=$5, has_crypto=$6, crypto_amount=$7, product_text=$8
        """, user_id, cmd, response, product_type, success_message, has_crypto, amount, product)

async def get_commands_db(user_id: int) -> Dict[str, dict]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT command, response, product_type, success_message, has_crypto, crypto_amount, product_text FROM commands WHERE user_id=$1", user_id)
        return {r['command']: {
            'response': r['response'], 
            'product_type': r['product_type'],
            'success_message': r['success_message'],
            'has_crypto': r['has_crypto'], 
            'crypto_amount': float(r['crypto_amount']) if r['crypto_amount'] else None, 
...
async def create_invoice_db(user_id: int, invoice_id: int, cmd: str, amount: float, product: str, chat_id: int, product_type: str = 'text', purchase_number: int = None):
    async with db_pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO crypto_invoices (user_id, invoice_id, command_name, amount, product_text, product_type, chat_id, purchase_number)
            VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
        """, user_id, invoice_id, cmd, amount, product, product_type, chat_id, purchase_number)
...
async def get_invoice_by_id_db(invoice_id: int) -> Optional[dict]:
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM crypto_invoices WHERE invoice_id=$1", invoice_id)
        return dict(row) if row else None
...
async def create_crypto_invoice(user_id: int, amount: float, description: str) -> Optional[dict]:
    ...
    data = {
        ...
        "description": "Оплата товара",  # Не пишем что именно продаем
...
def get_accounts_menu_keyboard():
    ...
    builder.button(text="Для продажи", callback_data="sell_accounts_menu", style="primary", icon_custom_emoji_id=eid("shop"))
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=eid("back"))
    builder.adjust(2, 1)
    return builder.as_markup()

def get_sell_accounts_menu_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="Добавить аккаунт", callback_data="add_sell_account", style="primary", icon_custom_emoji_id=eid("gift"))
    builder.button(text="Склад аккаунтов", callback_data="sell_stock", style="success", icon_custom_emoji_id=eid("box"))
...
def get_country_selection_keyboard():
    builder = InlineKeyboardBuilder()
    for country in AVAILABLE_COUNTRIES:
        builder.button(text=country, callback_data=f"sell_country_{country}", style="default")
    builder.button(text="Назад", callback_data="sell_accounts_menu", style="default", icon_custom_emoji_id=eid("back"))
    builder.adjust(3)
    return builder.as_markup()

def get_product_type_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="Текст", callback_data="product_type_text", style="primary", icon_custom_emoji_id=eid("file"))
    builder.button(text="Аккаунт", callback_data="product_type_account", style="success", icon_custom_emoji_id=eid("profile"))
...
def get_functions_keyboard():
    ...
    builder.button(text="Проверить покупку", callback_data="check_purchase", style="primary", icon_custom_emoji_id=eid("key"))
    builder.button(text="Назад", callback_data="main_menu", style="default", icon_custom_emoji_id=eid("back"))
    builder.adjust(1)
    return builder.as_markup()
...
async def get_2fa_code(client: TelegramClient) -> str:
    """Получает 2FA код из последнего сообщения"""
    try:
        messages = await client.get_messages("Telegram", limit=5)
        for msg in messages:
            if msg.text:
                # Ищем 5-значный код
...
@dp.callback_query(F.data == "sell_accounts_menu")
async def sell_accounts_menu_handler(callback: types.CallbackQuery):
    await safe_edit(callback.message, em('shop') + " <b>Аккаунты для продажи</b>\n\nДобавляйте аккаунты по странам", reply_markup=get_sell_accounts_menu_keyboard())
    await callback.answer()

@dp.callback_query(F.data == "add_sell_account")
async def add_sell_account_handler(callback: types.CallbackQuery, state: FSMContext):
    await safe_edit(callback.message, em('apps') + " <b>Добавление аккаунта для продажи</b>\n\n" + em('write') + " Введите номер телефона:\n<code>+79123456789</code>", reply_markup=get_back_keyboard("sell_accounts_menu"))
    await state.set_state(SellStates.waiting_for_sell_phone)
    await callback.answer()

@dp.message(SellStates.waiting_for_sell_phone)
async def sell_process_phone(message: types.Message, state: FSMContext):
    phone = message.text.strip()
    if not re.match(r'^\+\d{10,15}$', phone):
        await safe_send(message.chat.id, em('cross') + " Неверный формат."); return
    
    user_id = message.from_user.id
    client = TelegramClient('sessions/sell_' + str(user_id) + '_' + phone.replace("+", ""), API_ID, API_HASH)
    
    try:
        await client.connect()
        sent_code = await client.send_code_request(phone)
        # Сохраняем во временное хранилище
        if user_id not in sell_accounts: sell_accounts[user_id] = {}
        pending_logins[user_id] = {"client": client, "phone": phone, "phone_code_hash": sent_code.phone_code_hash, "is_sell": True}
        
        await safe_send(message.chat.id, em('gift') + " Код на <code>" + phone + "</code>\n" + em('write') + " Введите код:", reply_markup=get_back_keyboard("sell_accounts_menu"))
        await state.set_state(SellStates.waiting_for_sell_code)
    except Exception as ex:
        await client.disconnect()
        await safe_send(message.chat.id, em('cross') + " Ошибка: " + str(ex))
        await state.clear()

@dp.message(SellStates.waiting_for_sell_code)
async def sell_process_code(message: types.Message, state: FSMContext):
    code = message.text.strip()
    user_id = message.from_user.id
    
    if user_id not in pending_logins:
        await safe_send(message.chat.id, em('cross') + " Сессия истекла"); await state.clear(); return
    ...
    try:
        ...
        # Успешный вход - спрашиваем страну
        await state.update_data(sell_phone=data["phone"], sell_client_id=id(data["client"]))
        
        # Сохраняем клиент во временное хранилище
        if user_id not in sell_accounts: sell_accounts[user_id] = {}
        sell_accounts[user_id][data["phone"]] = {"client": data["client"], "phone": data["phone"]}
        
        await safe_send(message.chat.id, em('geo') + " <b>Выберите страну:</b>", reply_markup=get_country_selection_keyboard())
        await state.set_state(SellStates.waiting_for_sell_country)
        
        # Очищаем pending_logins
        pending_logins.pop(user_id, None)
        
    except SessionPasswordNeededError:
        await state.update_data(sell_phone=data["phone"])
        await safe_send(message.chat.id, em('lock') + " Требуется 2FA\n" + em('write') + " Введите пароль 2FA (будет сохранен):")
        await state.set_state(SellStates.waiting_for_sell_2fa)
    except Exception as ex:
...
@dp.message(SellStates.waiting_for_sell_2fa)
async def sell_process_2fa(message: types.Message, state: FSMContext):
    password = message.text.strip()
    user_id = message.from_user.id
    ...
    try:
        ...
        # Сохраняем 2FA пароль
        await state.update_data(sell_phone=data["phone"], sell_2fa=password)
        
        if user_id not in sell_accounts: sell_accounts[user_id] = {}
        sell_accounts[user_id][data["phone"]] = {"client": data["client"], "phone": data["phone"], "2fa_password": password}
...
@dp.callback_query(F.data.startswith("sell_country_"), SellStates.waiting_for_sell_country)
async def sell_select_country_handler(callback: types.CallbackQuery, state: FSMContext):
    country = callback.data.replace("sell_country_", "")
    data = await state.get_data()
    phone = data.get("sell_phone")
    twofa = data.get("sell_2fa", "")
    user_id = callback.from_user.id
    
    # Получаем клиент из хранилища
    account_data = sell_accounts.get(user_id, {}).get(phone)
    if not account_data:
    ...
    session_file = f'sessions/sell_{user_id}_{phone.replace("+", "")}.session'
    has_2fa = bool(twofa)
    
    # Сохраняем в БД
    await save_sell_account_db(user_id, country, phone, session_file, has_2fa, twofa if has_2fa else None)
    
    # Очищаем временное хранилище
    sell_accounts[user_id].pop(phone, None)
    
    await state.clear()
    await safe_edit(callback.message, em('check') + f" <b>Аккаунт {phone} добавлен!</b>\nСтрана: {country}\n2FA: {'Да' if has_2fa else 'Нет'}", reply_markup=get_sell_accounts_menu_keyboard())
    await callback.answer()

@dp.callback_query(F.data == "sell_stock")
async def sell_stock_handler(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    stock = await get_country_stock_db(user_id)
    
    if not stock:
        await safe_edit(callback.message, em('box') + " <b>Склад пуст</b>\n\nДобавьте аккаунты для продажи", reply_markup=get_sell_accounts_menu_keyboard())
        await callback.answer()
        return
    
    text = em('box') + " <b>Склад аккаунтов:</b>\n\n"
    for country in AVAILABLE_COUNTRIES:
        count = stock.get(country, 0)
        emoji_country = country
        text += f"{emoji_country}: <b>{count}</b> шт.\n"
    
    await safe_edit(callback.message, text, reply_markup=get_sell_accounts_menu_keyboard())
    await callback.answer()

# ============ ПРОВЕРКА ПОКУПКИ ============

@dp.callback_query(F.data == "check_purchase")
async def check_purchase_handler(callback: types.CallbackQuery, state: FSMContext):
    await safe_edit(callback.message, em('key') + " <b>Проверка покупки</b>\n\n" + em('write') + " Введите номер покупки:\n<code>.аккаунт 123</code>", reply_markup=get_back_keyboard("main_menu"))
    await state.set_state(CheckAccountStates.waiting_for_purchase_number)
    await callback.answer()

@dp.message(CheckAccountStates.waiting_for_purchase_number)
async def process_purchase_check(message: types.Message, state: FSMContext):
    text = message.text.strip().lower()
    
    # Парсим номер покупки
    purchase_match = re.search(r'\.аккаунт\s+(\d+)', text)
    if not purchase_match:
        await safe_send(message.chat.id, "Неверный формат. Используйте: .аккаунт НОМЕР")
        return
    
    purchase_number = int(purchase_match.group(1))
    user_id = message.from_user.id
    
    # Ищем проданный аккаунт
    sold_account = await get_sold_account_db(user_id, purchase_number)
    
    if not sold_account:
        await safe_send(message.chat.id, "Покупка с таким номером не найдена.")
        await state.clear()
        return
    
    # Создаем клиент для проверки
    session_file = sold_account['session_file']
    if not os.path.exists(session_file):
        await safe_send(message.chat.id, "Сессия не найдена. Обратитесь в поддержку.")
        await state.clear()
        return
    ...
    try:
        ...
        # Ищем самый новый чат
        newest_msg = None
        newest_date = None
        
        for dialog in dialogs:
            if dialog.message and dialog.is_user:
                if newest_date is None or dialog.message.date > newest_date:
                    newest_date = dialog.message.date
                    newest_msg = dialog.message
        
        # Получаем код из Telegram
        code = await get_2fa_code(client)
        
        # Формируем ответ (без премиум эмодзи)
        response = f"Номер покупки: {purchase_number}\n"
        response += f"Страна: {sold_account['country']}\n"
        response += f"Телефон: {sold_account['phone']}\n"
        
        if code:
            response += f"Код: {code}\n"
        
        if sold_account['has_2fa'] and sold_account['twofa_password']:
            response += f"Пароль 2FA: {sold_account['twofa_password']}\n"
        ...
        await client.disconnect()
        await safe_send(message.chat.id, f"Ошибка при проверке: {str(ex)[:200]}")
...
async def commands_menu_handler(callback: types.CallbackQuery):
    user_id = callback.from_user.id
...
async def process_crypto_token_handler(message: types.Message, state: FSMContext):
...
async def command_get_name_handler(message: types.Message, state: FSMContext):
    ...
    real_cmds = {k: v for k, v in existing.items() if not k.startswith('.оплатил_')}
    if cmd_name in real_cmds: await safe_send(message.chat.id, em('cross') + " Такая команда уже есть!"); return
    
    await state.update_data(command_name=cmd_name)
    
    # Сначала спрашиваем тип товара
    await safe_send(message.chat.id, em('write') + " <b>Выберите тип товара:</b>", reply_markup=get_product_type_keyboard())
    await state.set_state(CommandStates.waiting_for_product_type)

@dp.callback_query(F.data.startswith("product_type_"), CommandStates.waiting_for_product_type)
async def product_type_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit(callback.message, em('write') + " <b>Отправьте ответ на команду:</b>\n\nПлейсхолдеры: {NICK}, {FIRSTNAME}, {LASTNAME}, {FULLNAME}, {USERNAME}, {ID}\n" + em('money') + " {CRYPTOBOT} - ссылка, {RUB} - сумма\n" + em('geo') + " {UK}, {US}, {DE}, {FR}, {NL}, {CA}, {AU}, {IT}, {ES}, {PL} - наличие\nПоддерживается HTML и премиум эмодзи", reply_markup=get_back_keyboard("commands_menu"))
...
async def command_get_response_handler(message: types.Message, state: FSMContext):
    ...
    if has_crypto:
        ...
        if rub_match:
            ...
            await state.update_data(command_name=cmd_name, command_response=response, crypto_amount=crypto_amount, product_type=product_type)
            
            if product_type == "account":
                await safe_send(message.chat.id, em('write') + " <b>Введите текст успешной оплаты:</b>\n(плейсхолдеры: {NICK}, {FIRSTNAME}, страны, {PURCHASE_NUMBER})", reply_markup=get_back_keyboard("commands_menu"))
            else:
                await safe_send(message.chat.id, em('write') + " <b>Введите текст успешной оплаты:</b>\n(будет показан после оплаты)", reply_markup=get_back_keyboard("commands_menu"))
    ...
    # Сохраняем обычную команду
    await save_command_db(user_id, cmd_name, response, product_type)
    await state.clear()
...
async def command_get_product_handler(message: types.Message, state: FSMContext):
    success_message = message.html_text if message.html_text else message.text or ""
    data = await state.get_data()
    cmd_name = data.get("command_name", ".unknown")
    ...
    # Сохраняем Crypto команду
    await save_command_db(user_id, cmd_name, cmd_response, product_type, True, crypto_amount, None, success_message)
    
    # Создаем команду .оплатил_название
    paid_cmd = ".оплатил_" + cmd_name[1:]
    # Текст по умолчанию для неоплаченного счета
    unpaid_message = em('loading') + " <b>Оплата еще не поступила.</b>\nПроверьте оплату и попробуйте снова.\n\n" + em('info') + " Если вы только что оплатили, подождите 1-2 минуты."
    
    # Сохраняем команду .оплатил с кастомными сообщениями
    await save_command_db(user_id, paid_cmd, unpaid_message, product_type, True, crypto_amount, None, success_message)
...
async def setup_command_handler(user_id: int):
    ...
    commands_data = await get_commands_db(user_id)
    country_stock_data = await get_country_stock_db(user_id)
    ...
    async def command_handler(event):
        ...
        # Проверяем команду .аккаунт
        if msg_text.startswith('.аккаунт'):
            purchase_match = re.search(r'\.аккаунт\s+(\d+)', msg_text)
            if purchase_match:
                purchase_number = int(purchase_match.group(1))
                sold_account = await get_sold_account_db(user_id, purchase_number)
                ...
                if sold_account:
                    ...
                    if os.path.exists(session_file):
                        check_client = TelegramClient(session_file, API_ID, API_HASH)
                        try:
                            await check_client.connect()
                            code = await get_2fa_code(check_client)
                            
                            # Ответ без премиум эмодзи
                            response = f"Номер покупки: {purchase_number}\n"
                            response += f"Страна: {sold_account['country']}\n"
                            response += f"Телефон: {sold_account['phone']}\n"
                            ...
                            await client.send_message(event.sender_id, f"Ошибка при проверке: {str(ex)[:200]}")
                    else:
                        await client.send_message(event.sender_id, "Сессия не найдена. Обратитесь в поддержку.")
                else:
                    await client.send_message(event.sender_id, "Покупка с таким номером не найдена.")
            return
        ...
        command_data_extra = {"country_stock": country_stock_data}
        
        # Если команда с CryptoBot
        if cmd_data.get('has_crypto') and msg_text.startswith('.') and not msg_text.startswith('.оплатил'):
            token = await get_crypto_token(user_id)
            if not token:
                await client.send_message(event.sender_id, "Ошибка: не настроен Crypto Token")
                return
            
            amount = cmd_data.get('crypto_amount', 0)
            description = "Оплата товара"  # Не пишем что продаем в счете
            ...
            if invoice:
                ...
                # Получаем следующий номер покупки
                purchase_number = await increment_purchase_counter_db(user_id)
                
                await create_invoice_db(user_id, invoice['invoice_id'], msg_text, amount, description, 
                                       event.sender_id, cmd_data.get('product_type', 'text'), purchase_number)
                command_data_extra['purchase_number'] = purchase_number
            else:
                await client.send_message(event.sender_id, "Ошибка создания счета. Попробуйте позже.")
                return
        
        # Если команда .оплатил (можно редактировать текст)
        if msg_text.startswith('.оплатил'):
            ...
            if invoice_data:
                ...
                if status == 'paid':
                    ...
                    # Если тип товара - аккаунт, выдаем аккаунт
                    if invoice_data.get('product_type') == 'account':
                        purchase_number = invoice_data.get('purchase_number')
                        country = None
                        
                        # Определяем страну из команды
                        for c in AVAILABLE_COUNTRIES:
                            if "{" + c + "}" in original_cmd or c.lower() in original_cmd:
                                country = c
                                break
                        
                        if country:
                            accounts = await get_sell_accounts_db(user_id, country)
                            if accounts:
                                sell_acc = accounts[0]
                                await mark_account_sold_db(user_id, sell_acc['id'], purchase_number, event.sender_id)
                                
                                # Формируем ответ с номером покупки
                                success_msg = cmd_data.get('success_message', '')
                                if not success_msg:
                                    success_msg = "Оплата получена! Номер покупки: {PURCHASE_NUMBER}\nДля получения используйте: .аккаунт {PURCHASE_NUMBER}"
                                
                                success_msg = success_msg.replace("{PURCHASE_NUMBER}", str(purchase_number))
                                success_msg = success_msg.replace("{NICK}", user_info.get("first_name", ""))
                                
                                for c in AVAILABLE_COUNTRIES:
                                    success_msg = success_msg.replace("{" + c + "}", c)
                                
                                await client.send_message(event.sender_id, success_msg, parse_mode='html')
                                return
                    
                    # Для текстовых товаров - отправляем сохраненный текст
                    success_msg = cmd_data.get('success_message', '') or invoice_data.get('product_text', 'Спасибо за оплату!')
                    success_msg = success_msg.replace("{NICK}", user_info.get("first_name", ""))
                    await client.send_message(event.sender_id, success_msg, parse_mode='html')
                    return
                else:
                    # Отправляем кастомное сообщение (можно редактировать)
                    unpaid_msg = cmd_data.get('response', '')
                    if unpaid_msg:
                        await client.send_message(event.sender_id, unpaid_msg, parse_mode='html')
                    else:
                        await client.send_message(event.sender_id,
                            em('loading') + " <b>Оплата еще не поступила.</b>\nПроверьте оплату и попробуйте снова.\n\n" +
                            em('info') + " Если вы только что оплатили, подождите 1-2 минуты.", parse_mode='html')
                    return
...
async def welcome_setup_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    if welcome_data['is_enabled']:
        ...
        await callback.answer("Приветствие отключено!", show_alert=True); await commands_menu_handler(callback); return
...
async def edit_command_handler(callback: types.CallbackQuery):
    ...
    # Показываем и команду .оплатил если есть
    paid_cmd = ".оплатил_" + cmd_name[1:]
    has_paid = paid_cmd in commands_data
    
    builder = InlineKeyboardBuilder()
    if has_paid:
        builder.button(text="Ред. успешную", callback_data="edit_success_" + cmd_name, style="success", icon_custom_emoji_id=eid("check"))
        builder.button(text="Ред. неуспешную", callback_data="edit_unpaid_" + cmd_name, style="default", icon_custom_emoji_id=eid("loading"))
    ...
    info = ""
    if cmd.get('has_crypto'): info = em('money') + " Crypto: " + str(cmd.get('crypto_amount', 0)) + " RUB\n"
    if cmd.get('product_type'): info += em('tag') + " Тип: " + cmd.get('product_type', 'text') + "\n"
...
async def load_config_confirm_handler(callback: types.CallbackQuery):
    ...
    for cmd, data in cfg.get("commands", {}).items():
        await save_command_db(user_id, cmd, data.get('response', ''), data.get('product_type', 'text'),
                            data.get('has_crypto', False), data.get('crypto_amount'), 
                            data.get('product_text'), data.get('success_message'))
    welcome = cfg.get("welcome", {})
    await save_welcome_db(user_id, welcome.get('is_enabled', False), welcome.get('welcome_message'))
    await setup_command_handler(user_id)
    await callback.answer("Конфиг '" + config_name + "' загружен!", show_alert=True); await commands_menu_handler(callback)
...
async def parse_members_menu_handler(callback: types.CallbackQuery, state: FSMContext):
    if not get_active_account(callback.from_user.id): await callback.answer("Сначала добавьте аккаунт!", show_alert=True); return
...
async def process_parse_chat_handler(message: types.Message, state: FSMContext):
...
async def admin_stats_handler(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        total_sell = await conn.fetchval("SELECT COUNT(*) FROM sell_accounts WHERE is_sold=FALSE")
        total_sold = await conn.fetchval("SELECT COUNT(*) FROM sold_accounts")
    ...
    text = (
        ...
        + em('shop') + " На складе: " + str(total_sell) + "\n"
        + em('check') + " Продано: " + str(total_sold) + "\n"
