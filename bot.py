...
async def cmd_start(message: types.Message):
    ...
    text = (
        f"{em('bot')} <b>Добро пожаловать!</b>\n\n"
        f"<b>Главное меню:</b>\n"
        f"{em('settings')} <b>Менеджер аккаунтов</b> — управление аккаунтами\n"
        f"{em('stats')} <b>Функции</b> — рассылка, вступление в чаты\n"
        f"{em('megaphone')} <b>Поддержка</b> — связь с поддержкой"
    )
...
async def accounts_manager(message: types.Message):
    ...
    text = (
        f"{em('settings')} <b>Менеджер аккаунтов</b>\n"
        f"{em('profile')} Активных аккаунтов: {count} (безлимит)\n"
        f"{em('user_check')} Выбран: <code>{selected}</code>"
    )
...
async def functions_menu(message: types.Message):
    text = (
        f"{em('stats')} <b>Функции</b>\n"
        f"{em('megaphone')} Рассылка сообщений\n"
        f"{em('people')} Вступление в чаты"
    )
...
async def support(message: types.Message):
    text = (
        f"{em('megaphone')} <b>Поддержка</b>\n"
        f"{em('link')} Свяжитесь с нами: {SUPPORT_USERNAME}"
    )
...
async def admin_panel(message: types.Message):
    ...
    text = (
        f"{em('lock')} <b>Админ панель</b>\n"
        f"{em('stats')} Статистика бота\n"
        f"{em('megaphone')} Рассылка всем пользователям"
    )
...
async def add_account(callback: types.CallbackQuery, state: FSMContext):
    text = (
        f"{em('apps')} <b>Добавление аккаунта</b>\n"
        f"{em('write')} Введите номер телефона: <code>+79123456789</code>"
    )
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        text = (
            f"{em('gift')} Код отправлен на <code>{phone}</code>\n"
            f"{em('write')} Введите код из SMS:"
        )
...
async def process_code(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        text = (
            f"{em('lock')} Требуется 2FA\n"
            f"{em('write')} Введите пароль:"
        )
        await safe_send(message.chat.id, text, reply_markup=get_back_keyboard("accounts_manager"))
...
async def my_accounts(callback: types.CallbackQuery):
    ...
    text = (
        f"{em('profile')} <b>Мои аккаунты</b>\n"
        f"{em('unlock')} Всего: {len(accounts)}\n"
        f"Выберите аккаунт:"
    )
...
async def select_account_menu(callback: types.CallbackQuery):
    ...
    selected = user_selected_account.get(user_id, "Не выбран")
    text = (
        f"{em('user_check')} <b>Выбор аккаунта</b>\n"
        f"Текущий: <code>{selected}</code>\n\n"
        f"Выберите аккаунт для работы:"
    )
    await safe_edit(callback.message, text, reply_markup=builder.as_markup())
    await callback.answer()
...
async def account_info(callback: types.CallbackQuery):
    ...
    text = (
        f"{em('profile')} <b>Аккаунт:</b> <code>{phone}</code>\n"
        f"{em('check')} Статус: {status}"
    )
    await safe_edit(callback.message, text, reply_markup=builder.as_markup())
    await callback.answer()
...
async def new_broadcast(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"{em('write')} <b>Отправьте сообщения для рассылки (до 5):</b>\n"
        f"Каждое сообщение с новой строки\n"
        f"<code>Сообщение 1\nСообщение 2\nСообщение 3</code>\n\n"
        f"{em('info')} Поддерживается HTML:\n"
        f"• <b>жирный</b> • <i>курсив</i> • <code>код</code>\n"
        f"• <u>подчеркнутый</u> • <s>зачеркнутый</s>\n"
...
async def create_chat_selection_message(chat_id: int, state: FSMContext, page: int):
    ...
    text = (
        f"{em('people')} <b>Выберите чаты ({limit_text}):</b>\n"
        f"Выбрано: {len(selected)}/10 | Страница {page + 1}/{total_pages}\n\n"
        f"{em('info')} После выбора чата появится новое сообщение\n"
        f"{em('send')} Можно запустить от 1 чата"
    )
...
async def choose_broadcast_mode(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"{em('send')} <b>Выберите режим рассылки:</b>\n\n"
        f"{em('loading')} <b>Одновременная</b> — сначала отправляет во все чаты, потом ждет задержку\n"
        f"{em('loading')} <b>Рандомная</b> — отправляет в случайные чаты с задержкой"
    )
...
async def start_broadcast_execution(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"{em('send')} <b>Рассылка запущена!</b>\n"
        f"{em('people')} Чатов: {len(selected_chats)}\n"
        f"{em('file')} Сообщений: {len(data.get('messages_list', ['']))} (в каждый: {data.get('message_count', 1)})\n"
        f"{em('clock')} Задержка: {data.get('delay', 1)}с\n"
        f"{em('loading')} Режим: {mode_text}"
    )
    await safe_edit(callback.message, text, reply_markup=builder.as_markup())
    await state.clear()
...
async def join_chats_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"{em('people')} <b>Вступление в чаты</b>\n\n"
        f"{em('write')} Отправьте список юзернеймов (каждый с новой строки):\n"
        f"<code>@chat1\n@chat2\n@chat3</code>\n\n"
        f"{em('info')} Задержка между вступлениями: 15 секунд"
    )
...
async def process_join_usernames(message: types.Message, state: FSMContext):
    ...
    for i, username in enumerate(lines, 1):
        ...
        if i % 3 == 0 or i == len(lines):
            progress_text = (
                f"{em('loading')} <b>Вступление в чаты...</b>\n"
                f"Прогресс: {i}/{len(lines)}\n"
                f"{em('check')} Вступил: {joined}\n"
    ...
    result_text = (
        f"{em('celebration')} <b>Вступление завершено!</b>\n\n"
        f"{em('check')} Успешно: {joined}\n"
        f"{em('cross')} Ошибок: {failed}"
    )
...
async def admin_stats(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"{em('profile')} Аккаунтов: {total_accounts}\n"
        f"{em('clock')} Активных рассылок: {active_broadcasts_count}\n"
        f"{em('megaphone')} Поддержка: {SUPPORT_USERNAME}"
    )
...
async def admin_broadcast_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"{em('megaphone')} <b>Рассылка всем пользователям</b>\n\n"
        f"{em('write')} Отправьте сообщение с поддержкой HTML:\n"
        f"• <b>жирный</b> • <i>курсив</i> • <code>код</code>\n"
        f"• <u>подчеркнутый</u> • <s>зачеркнутый</s>\n"
        f"• <blockquote>цитата</blockquote>\n"
        f"• Премиум эмодзи"
...
async def admin_broadcast_get_message(message: types.Message, state: FSMContext):
    ...
    text = (
        f"{em('send')} <b>Подтвердите рассылку</b>\n\n"
        f"{em('people')} Пользователей: {total_users}\n"
        f"{em('file')} Сообщение:\n"
        f"<blockquote>{msg_html[:200]}{'...' if len(msg_html) > 200 else ''}</blockquote>\n\n"
        f"Отправить всем?"
    )
